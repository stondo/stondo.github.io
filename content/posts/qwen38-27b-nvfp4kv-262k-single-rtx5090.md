...
Three takeaways. The everything-FP4 checkpoint keeps both crowns — its lighter weights feed the KV pool *and* the decode path (the full-vocab lm_head in FP4 is not a rounding error). The unsloth build on this recipe gains context (106K → 196K on my old fp8-KV setup) but no speed: FP8-attention dequant dominates. And the pleasant surprise: the **abliterated heretic build holds the full 262,144-token window too** — the uncensored lane used to cap at 127K, so that's a straight 2x for a model family that usually eats a quality-or-context tax. (I'd first tried rolling my own uncensored NVFP4 quant for this lane — [that story](/posts/obliteratus-qwen38-27b-nvfp4-rtx5090/) is a fun ModelOpt minefield tour, but the build has a looping issue under investigation, so the third-party checkpoint above is the lane I actually run.)

One more surprise from the heretic build, because uncensored-and-multimodal is exactly the combination you'd want together: on that checkpoint, **the MTP draft and the vision encoder refuse to coexist**. Boots with both enabled die inside the encoder's profiling pass with a delightfully vague `NoneType has no attribute 'size'`; drop speculation and the same boot comes up clean — and vision then tests *correctly* (a generated report-card image came back with the title text read, all three shapes named, colors and relative positions right). The gittensor checkpoint runs both together without complaint, so it's a checkpoint-specific interaction, not a recipe bug. The unit ships with a one-line profile switch: text-fast (MTP, 91 t/s) by default, vision (no MTP, ~30% slower decode — but the pool *grows* to 449K tokens, since the draft's KV group stops existing) when you flip it. For the record, the unsloth build's multimodal path wouldn't even finish its profiler on this vLLM build — of the three checkpoints, exactly one has working vision here, and it's the one I expected least.
