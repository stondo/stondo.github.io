...
## Footnote: GLM-5.3-Flash still does not fit

Same session, same box, I also tried GLM-5.3-Flash (image pulled, the 181 GB LibertAI stock NVFP4 checkpoint on disk, quadlet ready). It OOMs **at weight load** on TP2: 90.5 GiB of weights per GPU plus a 3.4 GiB Marlin NVFP4-MoE conversion workspace against 95.01 GiB of capacity. About a gigabyte short, and no context or sequence tuning touches the load phase. The uncensor-CRACK build has the same tensor layout and hits the same wall. It stays parked until either SGLang's `glm5_next` support lands with a different memory profile or llama.cpp-format builds appear (the unsloth repo is still an empty placeholder as I write this). Weekly recheck, zero urgency.
